-- Islamic Learning Platform Database Logic (Fixed Version 2)
...
-- Trigger to set default stage and page for new students (Fixed)
...
-- Procedure to notify users (must be created first as it's used by other procedures)
...
-- Procedure to set student stage (Fixed variable declaration placement)
...
BEGIN
    ...
    -- Notify teacher about the rating
...
BEGIN
    ...
    DECLARE student_cursor CURSOR FOR 
        SELECT s.user_id 
        FROM students s
        WHERE s.current_stage_id = NEW.stage_id
        OR EXISTS (
            SELECT 1 FROM assignment_targets at 
            JOIN group_members gm ON at.group_id = gm.group_id 
            WHERE at.assignment_id = NEW.id AND gm.student_id = s.id
        );
    ...
    OPEN student_cursor;
    read_loop: LOOP
        FETCH student_cursor INTO v_student_user_id;
        IF done THEN
            LEAVE read_loop;
        END IF;
        
        CALL notify_scope(v_student_user_id, 'NEW_ASSIGNMENT', 
            CONCAT('واجب جديد: ', NEW.title));
    END LOOP;
    CLOSE student_cursor;
END //
DELIMITER ;

-- Trigger for material notifications
...
BEGIN
    ...
    DECLARE student_cursor CURSOR FOR 
        ...
        OR EXISTS (
            ...
            WHERE gm.group_id = NEW.group_id AND gm.student_id = s.id
        );
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    ...
    read_loop: LOOP
        ...
        CALL notify_scope(v_student_user_id, 'NEW_MATERIAL', 
            CONCAT('مادة جديدة: ', NEW.title));
    END LOOP;
    CLOSE student_cursor;
END //
DELIMITER ;

-- Trigger for meeting notifications
...
BEGIN
    ...
    DECLARE student_cursor CURSOR FOR 
        SELECT s.user_id 
        FROM students s
        WHERE s.current_stage_id = NEW.level_stage_id
        OR EXISTS (
            SELECT 1 FROM group_members gm 
            WHERE gm.group_id = NEW.group_id AND gm.student_id = s.id
        );
    ...
    read_loop: LOOP
        ...
        CALL notify_scope(v_student_user_id, 'NEW_MEETING', 
            CONCAT('اجتماع جديد: ', NEW.title));
    END LOOP;
    CLOSE student_cursor;
END //
DELIMITER ;

-- Trigger for certificate approval notifications
...
-- Trigger for submission notifications to teacher
